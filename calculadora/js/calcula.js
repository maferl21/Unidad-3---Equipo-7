...
function seno() {
    x = Math.sin(x)//calcula el seno en radianes   (x*(Math.PI/180)) ---->grados
    pantalla.innerHTML = x; //mostrar en pantalla resultado
    op = "no"; //quitar operaciones pendientes.
    xi = 1; //se puede reiniciar la pantalla
}
function coseno() {
    x = Math.cos(x * (Math.PI / 180)) //calcula el coseno
    pantalla.innerHTML = x; //mostrar en pantalla resultado
    op = "no"; //quitar operaciones pendientes.
    xi = 1; //se puede reiniciar la pantalla
}
function tangente() {
    x = Math.tan(x * (Math.PI / 180)) //calcular Tangente
    pantalla.innerHTML = x; //mostrar en pantalla resultado
    op = "no"; //quitar operaciones pendientes.
    xi = 1; //se puede reiniciar la pantalla
}
function Aseno() {
    x = Math.asin(x) //calcula el arco seno en radianes   *180/Math.PI----> en grados
    pantalla.innerHTML = x; //mostrar en pantalla resultado
    op = "no"; //quitar operaciones pendientes.
    xi = 1; //se puede reiniciar la pantalla
}
function Acoseno() {
    x = Math.acos(x) //calcula el arco coseno
    pantalla.innerHTML = x; //mostrar en pantalla resultado
    op = "no"; //quitar operaciones pendientes.
    xi = 1; //se puede reiniciar la pantalla
}
function Atangente() {
    x = Math.atan(x) //calcula el arc
    pantalla.innerHTML = x; //mostrar en pantalla resultado
    op = "no"; //quitar operaciones pendientes.
    xi = 1; //se puede reiniciar la pantalla
}
...
function conver(base){
    var decimalTemporal = parseInt(x, 10); // Desde la base 2
    var resultado;
    switch(base){
        case 'bin': resultado = decimalTemporal.toString(2);  break;
        case 'oc': resultado = decimalTemporal.toString(8);  break;
        case 'hex': resultado = decimalTemporal.toString(16);  break;
    }
    x=resultado;
    pantalla.innerHTML = x; //mostrar en pantalla resultado
    op = "no"; //quitar operaciones pendientes.
